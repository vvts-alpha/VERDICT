...
The Electron desktop shell hosts `@veritas/server` in-process on an ephemeral loopback port and runs assessments through its packaged CLI using `ELECTRON_RUN_AS_NODE`. Runtime versions come from the pinned desktop dependency and are checked in each artifact manifest; do not duplicate version overrides. State uses Node's builtin `node:sqlite`; the optional Claude path also ships a platform-specific executable. Automation uses an external Chromium. Settings and assessments live under the OS userData directory. Build, Windows artifact checks, and promotion must follow [docs/RELEASING.md](docs/RELEASING.md) and [apps/desktop/PACKAGING.md](apps/desktop/PACKAGING.md). Do not use an unfrozen deploy or replace the tested CI installer with a local rebuild. Existing release authorization is sufficient; do not ask for repeated confirmation.
