...
- **Brand is "AMRAAM" but internal names are intentionally kept.** The product/display brand and the traffic marker (`x-amraam`) were renamed from VERITAS, but the **package scope `@veritas/*`**, the **MCP namespace `mcp__veritas__`** (tool prefix in `STAGE_TOOLS`/`allowedTools`), and the **`VERITAS_BROWSER_PATH`** env var are left as-is — they're invisible to users and renaming them is pure breakage risk. Don't "fix" this inconsistency.
