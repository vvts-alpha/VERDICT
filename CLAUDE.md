...
| Command | Purpose |
|---|---|
| `pilot` | **Claude-led, staged** assessment (survey → methodology → diagnosis). `--manifest`/`--url`, `--model` (deep) + `--fast-model` (model tiering), `--max-turns`, `--headed`, `--burp-proxy <url>` (route all traffic through Burp; off by default = unchanged). |
...
- **Auth = credentials only, no cookie injection.** A manifest carries `auth.roles: [{name, pass}]`; the agent auto-discovers the login URL + form fields (`smartLogin`) and re-crawls authenticated. MFA/CAPTCHA/challenge → `detectStuck` raises a non-blocking `HumanHandoff` (headed browser, human finishes login in the live profile). Never inject cookies/tokens. `roles[0]` = primary login; multiple roles drive `authDiffScreen`.
- **Brand is "Umbra Hands" but internal names are intentionally kept.** The product/display brand and the traffic marker (`x-umbra-hands`) were renamed from VERITAS, but the **package scope `@veritas/*`**, the **MCP namespace `mcp__veritas__`** (tool prefix in `STAGE_TOOLS`/`allowedTools`), and the **`VERITAS_BROWSER_PATH`** env var are left as-is — they're invisible to users and renaming them is pure breakage risk. Don't "fix" this inconsistency.
...
No linter/formatter is configured. Match nearby code: ESM, 4-space-ish existing style, kebab-case filenames, PascalCase types, camelCase functions. Bilingual is normal — code/identifiers English, many comments Japanese.
