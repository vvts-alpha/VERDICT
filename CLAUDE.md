...
| `pilot --from-asr <asr-id\|asset_inventory.json>` | **P3 wide→deep handoff**: promote an ASR run's top-ranked assets into per-host deep pilot runs (`--from-asr-top`/`--from-asr-band`/`--from-asr-concurrency`). Scope is PINNED to the ASR run's authorization boundary (may only narrow). |
| `asr --domain <apex\|*.wildcard>` | **Attack Surface Recon (Phase-0, `@veritas/asr`)**: passive CT discovery (crt.sh, forced IPv4) + optional `subfinder`/`--brute` (dnsx/native) + `--import` → scope-filter → dns/HTTP liveness → deterministic `scoreAsset` rank (+`--triage` AI) → subdomain-takeover check → `runs/<id>/asset_inventory.json` (the second contract artifact; Asset schema in `core/types/asset.ts`). Recon findings persist as `suspected` leads. Fails LOUD on a crt.sh outage (`--allow-degraded` to continue). |
...
**Three execution modes over one shared substrate**, optionally fronted by a Phase-0 recon (`asr`). pilot is the newer/primary direction (see `README` / memory); the deterministic pipeline is the granular/inspectable path; `redteam` is the LLM-assistant mode.
...
**Phase-0 (optional, to the LEFT of survey): `asr` — Attack Surface Recon** (`packages/asr`): a wildcard/apex → subdomain discovery (crt.sh + subfinder/brute/import) → scope-filter → liveness → deterministic `scoreAsset` rank (+AI triage) → `asset_inventory.json` (the **second contract artifact**, `core/types/asset.ts`). `pilot --from-asr` promotes ranked assets into per-host deep runs (scope pinned, may only narrow).

**Phases joined by contract artifacts.** The main coupling point: Phase 1 (recon + labeling) writes **`screen_inventory.json`** (the `Screen` schema in `core/types/screen.ts`); Phase 2 (scan + business logic) and the WebUI only read it — keep it stable. ASR adds a **second contract artifact**, **`asset_inventory.json`** (the `Asset`/`AssetInventory` schema in `core/types/asset.ts`), which `pilot --from-asr` consumes to seed per-host runs.
...
cli ── orchestrates everything (pilot / assess / asr / redteam / serve …)
pilot ─ agent ─ scanner ─┐
asr ─ llm-attacks ───────┤   (asr → core+llm ; llm-attacks → core+scanner)
crawler ─ llm ───────────┤
...
- `asr` — Attack Surface Recon (Phase-0): CT/subdomain discovery (`discovery.ts` crt.sh forced-IPv4, `sources.ts` subfinder/dnsx, `brute.ts` native, `import.ts`), `scoreAsset` (`score.ts`), `takeover.ts`, `io.ts` (`asset_inventory.json`). External tools run via `execFileRunTool` (no shell, hardcoded argv). Deps: core + llm only.
...
- **Evidence discipline** (`scanner`): a finding is `confirmed` only with a **negative control that fails + ≥2 positive replays that succeed**; catch-all / 0-byte-200 / unstable responses → `refuted`. `agent`, `pilot`, and `llm-attacks` all route confirmations through this — never mark `confirmed` by hand. The **OOB path** (`probe_oob`) is a distinct callback oracle but STILL rides this contract (a benign control + ≥2 positive callbacks). **ASR** recon findings are always `suspected` leads, never `confirmed`. Pilot's post-diagnosis **findings-QA** (`triagePilotFindings`) can only **DEMOTE** (never promote) — the deterministic evidence floor stays the confirm authority, so an over-confident model can't talk a fake into `confirmed`.
