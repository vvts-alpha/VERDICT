...
6. For an authorized release, run the same command with `publish`. No repeated permission question is needed. The tool creates/resumes a matching draft, uploads files, verifies GitHub's stored sizes and hashes, rechecks CI, publishes, and verifies the latest release. Upload failure leaves a draft. Published assets are immutable; corrections need a newer version.
